...
3. **Domain Constraints** -- 16 emergency patterns, 4 forbidden content patterns, dosage prohibition, no diagnoses
...
**Unit Tests (68 tests, <2s, no API keys needed):**
...
| `test_verifier.py` | 22 | Hallucination regex patterns, confidence scoring math, 16 emergency patterns, 4 forbidden content patterns, output validation, full pipeline with post-processing |
...
Result: **68 passed, 0 failed, 1.62s**
...
| Adversarial/Safety | 12 | Emergency escalation, prompt injection, dosage requests, role switching, bypass attempts, overdose |
...
- Isolated unit tests (68 tests) run in <2 seconds with zero external dependencies
